...
QUnit.test("Allow extra", function(){
	...
	F.wait(1, function() {
		QUnit.equal(can.viewModel($('bh-suggestions')).attr('val'), 'foo/bar', 'Val is set to extra value');
	});
});
...
QUnit.test("Selecting suggestion", function(){
	...
	F.wait(1, function(){
		QUnit.equal(can.viewModel($('bh-suggestions')).attr('val'), 'aljosa/muchi.me', 'Suggestion is set on scope');
	});
});
